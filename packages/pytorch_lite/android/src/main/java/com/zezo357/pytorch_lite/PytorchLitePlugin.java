...
public class PytorchLitePlugin implements FlutterPlugin, Pigeon.ModelApi {
  ...
  public Long loadModel(String modelPath, Long numberOfClasses, Long imageWidth, Long imageHeight) {
    ...
    try {
      ...
      if (numberOfClasses != null && imageWidth != null && imageHeight != null) {
        prePostProcessors
            .add(new PrePostProcessor(numberOfClasses.intValue(), imageWidth.intValue(), imageHeight.intValue()));
      } else {
  ...
  public void getPredictionCustom(Long index, List<Double> input, List<Long> shape, String dtype,
    ...
    try {
      ...
      Log.e(TAG, "Your input type " + dtype_enum.toString().toLowerCase() + " ("
          + Convert.dtypeAsPrimitive(dtype.toString()) + ") " + "does not match with model input type", e);
      result.success(null);
    }
